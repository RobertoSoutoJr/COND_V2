...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao']) && $_POST['acao'] === 'mudar_preco') {
    try {
        ...
        $novo_preco = str_replace(['R$', ' '], '', $_POST['novo_preco']);
        ...
        if ($novo_preco >= 0) {
            ...
            // Recarrega a página para recalcular os totais
            header("Location: condicionais_detalhes.php?id=$cond_id&msg=preco_ok");
            exit;
        }
    } catch (PDOException $e) {
        $mensagem = "<div class='bg-red-100 text-red-700 p-4 rounded mb-4'>Erro ao atualizar preço: " . $e->getMessage() . "</div>";
    }
}
// Mensagem de sucesso
if (isset($_GET['msg']) && $_GET['msg'] === 'preco_ok') {
    $mensagem = "<div class='bg-green-100 text-green-700 p-4 rounded mb-4'>Preço do item atualizado com sucesso!</div>";
}
...
<body class="bg-gray-100 pb-20">
    ...
    <div class="container mx-auto mt-10 px-4">
        ...
        <a href="condicionais_lista.php" class="text-blue-600 hover:underline mb-4 inline-block">&larr; Voltar para a Lista</a>
        ...
        <div class="bg-white p-8 rounded-lg shadow-lg">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div class="bg-blue-50 p-4 rounded-lg text-center">
                    <p class="text-sm text-blue-800 font-bold uppercase">Total da Sacola</p>
                    <p class="text-2xl font-bold text-blue-900">R$ <?= number_format($total_sacola, 2, ',', '.') ?></p>
                </div>
                <div class="bg-green-50 p-4 rounded-lg text-center">
                    <p class="text-sm text-green-800 font-bold uppercase">Total Vendido</p>
                    <p class="text-2xl font-bold text-green-900">R$ <?= number_format($total_vendido, 2, ',', '.') ?></p>
                </div>
                <div class="bg-gray-100 p-4 rounded-lg text-center">
                    <p class="text-sm text-gray-600 font-bold uppercase">Devolvido</p>
                    <p class="text-2xl font-bold text-gray-800">R$ <?= number_format($total_devolvido, 2, ',', '.') ?></p>
                </div>
                <div class="bg-green-100 p-4 rounded-lg text-center border border-green-300">
                    <p class="text-sm text-green-800 font-bold uppercase">Lucro Apurado</p>
                    <p class="text-2xl font-bold text-green-900">R$ <?= number_format($lucro_apurado, 2, ',', '.') ?></p>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4 text-center">
                                <span id="texto-preco-<?= $item['id'] ?>" class="font-bold text-base">
                                ...
                                <form method="POST" id="form-preco-<?= $item['id'] ?>" class="hidden">
                                    ...
                                    <input type="text" name="novo_preco" 
                                           value="<?= number_format($item['preco_momento'], 2, ',', '.') ?>"
                                           class="w-24 text-center font-bold border rounded py-1 px-2">
                                ...
                                <?php if ($condicional['status'] !== 'FINALIZADO'): // Só permite editar se a sacola estiver aberta ?>
                                    <button id="btn-preco-<?= $item['id'] ?>" onclick="toggleEdit(<?= $item['id'] ?>)" 
                                            class="text-blue-600 hover:underline text-xs ml-2">[Alterar]</button>
                                <?php endif; ?>
                            </td>
